...
def downcast_dtypes(df, from_bytes=64, to_bytes=32, columns=None):
    ...
    :param columns: list of columns to cast
    ...
    float_cols = [c for c in df if (df[c].dtype == f'float{from_bytes}' and c in columns)]
    int_cols =   [c for c in df if (df[c].dtype == f'int{from_bytes}' and c in columns)]
